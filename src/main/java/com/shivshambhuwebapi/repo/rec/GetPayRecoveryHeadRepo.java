...
public interface GetPayRecoveryHeadRepo extends JpaRepository<GetPayRecoveryHead, Integer> {
	...
	@Query(value = "SELECT h.*,c.cust_name,c.cust_mob_no FROM t_pay_recovery_header h ,m_customer c,t_bill_header b WHERE h.del_status=1 AND h.status=0 AND h.credit_date2 Between :fromDate AND :toDate AND c.cust_id=h.cust_id AND  h.bill_head_id=b.bill_head_id ", nativeQuery = true)
	List<GetPayRecoveryHead> getPayRecBetDate(@Param("fromDate") String fromDate, @Param("toDate") String toDate);


	@Query(value = "SELECT h.*,c.cust_name,c.cust_mob_no FROM t_pay_recovery_header h ,m_customer c,t_bill_header b WHERE h.del_status=1 AND h.status=0 AND h.credit_date2 Between :fromDate AND :toDate AND c.cust_id=h.cust_id AND  h.bill_head_id=b.bill_head_id  AND  b.ex_int1=:plantId ", nativeQuery = true)
	...
	@Query(value = "SELECT h.*,c.cust_name,c.cust_mob_no FROM t_pay_recovery_header h ,m_customer c,t_bill_header b WHERE h.del_status=1 AND h.status=0  AND c.cust_id=h.cust_id  AND  h.bill_head_id=b.bill_head_id  AND  b.ex_int1=:plantId ", nativeQuery = true)
