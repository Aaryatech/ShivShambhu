...
public interface GetMatIssueHeaderRepo extends JpaRepository<GetMatIssueHeader, Integer> {
	...
	@Query(value = "SELECT c.*,n.contr_name ,n.contr_mob  FROM t_mat_issue_contra_header c,m_contractor n WHERE c.del_status=1 AND c.contr_id=n.contr_id ORDER BY c.mat_header_id DESC", nativeQuery = true)
	List<GetMatIssueHeader> getMatIssueHeadeList();

	@Query(value = "SELECT c.*,n.contr_name ,n.contr_mob  FROM t_mat_issue_contra_header c,m_contractor n WHERE c.del_status=1 AND c.contr_id=n.contr_id AND c.mat_header_id=:matHeaderId ORDER BY c.mat_header_id DESC", nativeQuery = true)
	...
	@Query(value = "SELECT c.mat_header_id,c.issue_no,c.contr_id,c.date,c.user_id,SUM(c.total)as total,SUM(c.qty_total) as qty_total,"
			...
			+ "c.contr_id=n.contr_id AND  c.date  BETWEEN  :fromDate AND :toDate GROUP BY c.contr_id ORDER BY c.mat_header_id DESC", nativeQuery = true)
