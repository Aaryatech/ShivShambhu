...
public interface GetChalanHeaderDetailRepo extends JpaRepository<GetChalanHeaderDetail, Integer> {
	...
	@Query(value = "  SELECT\n" + "        t_chalan_header.*,\n" + "        m_vehicle.veh_no,\n"
			...
			+ "        m_item_fg.item_code,\n" + "          m_item_fg.item_rate1,\n"
			+ "        m_uom.uom_name,m_uom.uom_short_name\n" + "        \n" + "        \n" + "        \n"
			+ "    FROM\n" + "        m_vehicle,\n" + "        m_plant,\n" + "        m_company,\n"
			+ "        m_customer,\n" + "        m_project,\n"
			+ "        t_chalan_header     ,t_chalan_detail,m_item_fg,m_uom  \n" + "    WHERE\n"
			...
			+ "        AND  t_chalan_detail.item_id=m_item_fg.item_id AND m_item_fg.uom_id=m_uom.uom_id\n"
	...
	@Query(value = "  SELECT\n" + "        t_chalan_header.*,\n" + "        m_vehicle.veh_no,\n"
			...
			+ "        AND t_chalan_header.plant_id=:plantId\n" + "        \n"
			+ "        AND t_chalan_header.ex_int1=1  \n"
			+ "   AND  t_chalan_header.ex_float1=0     AND t_chalan_header.status!=2 \n"
			+ "        AND  t_chalan_detail.item_id=m_item_fg.item_id AND m_item_fg.uom_id=m_uom.uom_id\n"
			+ "        AND t_chalan_detail.chalan_id=t_chalan_header.chalan_id\n" + "    ORDER BY\n"
			+ "        t_chalan_header.chalan_id DESC  \n" + "", nativeQuery = true)
