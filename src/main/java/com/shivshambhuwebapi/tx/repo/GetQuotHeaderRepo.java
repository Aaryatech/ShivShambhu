...
public interface GetQuotHeaderRepo extends JpaRepository<GetQuotHeader, Integer> {
	...
	@Query(value = " SELECT h.*, c.cust_name ,u.usr_name,o.comp_name,p.proj_name,COALESCE((SELECT t.pay_term FROM  m_payment_term "
			+ "t WHERE t.pay_term_id=h.pay_term_id ),'NULL') as pay_term,pn.plant_name FROM t_quot_header h,m_customer c ,m_user"
			+ " u,m_company o ,m_project p ,m_plant pn WHERE h.del_status=1 AND h.cust_id=c.cust_id AND u.user_id=h.user_id AND "
			+ "h.company_id=o.company_id AND p.proj_id=h.proj_id  AND h.plant_ids=:plantId AND h.quot_date BETWEEN :fromDate AND "
			+ ":toDate AND pn.plant_id=h.plant_ids", nativeQuery = true)
	List<GetQuotHeader> getQuotHeaderByPlantId(@Param("plantId") int plantId, @Param("fromDate") String fromDate,
			@Param("toDate") String toDate);

	@Query(value = "SELECT h.*, c.cust_name ,u.usr_name,o.comp_name,p.proj_name,COALESCE((SELECT t.pay_term FROM  m_payment_term "
			+ "t WHERE t.pay_term_id=h.pay_term_id ),'NULL') as pay_term,pn.plant_name FROM t_quot_header h,m_customer c ,m_user u,"
			+ "m_company o ,m_project p ,m_plant pn WHERE h.del_status=1 AND h.cust_id=c.cust_id AND u.user_id=h.user_id AND "
			+ "h.company_id=o.company_id AND p.proj_id=h.proj_id  AND h.plant_ids=:plantId AND h.quot_date BETWEEN :fromDate AND"
			+ " :toDate AND pn.plant_id=h.plant_ids AND h.cust_id=:custId", nativeQuery = true)
