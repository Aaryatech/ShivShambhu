...
public interface GetVehDetailRepo extends JpaRepository<GetVehDetail, Integer> {
	...
	@Query(value = "select t_issue_veh_detail.*,i.item_desc,i.item_code ,u.uom_name from t_issue_veh_detail ,m_item i,m_uom u  "
			+ "where mat_veh_header_id in (select mat_veh_header_id from t_issue_veh_header where veh_id in(:vehicleId)) AND"
			+ " t_issue_veh_detail.item_id=i.item_id  AND u.uom_id=t_issue_veh_detail.uom_id", nativeQuery = true)
	List<GetVehDetail> getMatIssueByVehicleId(@Param("vehicleId") int vehicleId);
}
