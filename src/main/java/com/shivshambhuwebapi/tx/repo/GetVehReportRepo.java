...
public interface GetVehReportRepo extends JpaRepository<GetVehReport, Integer> {
	...
	@Query(value = "SELECT h.mat_veh_header_id,h.veh_no,h.veh_id,h.date,h.user_id,SUM(h.veh_total) AS veh_total,SUM(h.veh_qty_total) "
			...
			+ "  FROM t_weighing w WHERE w.del_status=1  AND w.veh_id=h.veh_id AND w.date BETWEEN :fromDate AND :toDate),'0') as weigh_contr_qty  "
			+ "FROM t_issue_veh_header h,m_vehicle v  WHERE h.del_status=1 AND h.veh_id=v.vehicle_id AND h.date "
