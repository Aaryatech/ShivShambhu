...
public interface GetChalanDetailRepo extends JpaRepository<GetChalanDetail, Integer> {
	...
	@Query(value = " SELECT t_chalan_detail.*, m_item_fg.item_name,m_item_fg.item_code,m_uom.uom_name,"
			+ " t_order_detail.rem_ord_qty,t_order_detail.order_qty, "
			+ " m_uom.uom_short_name " + " FROM t_chalan_detail,m_item_fg,m_uom,t_order_detail "
			+ " WHERE  t_chalan_detail.item_id=m_item_fg.item_id AND m_item_fg.uom_id=m_uom.uom_id "
