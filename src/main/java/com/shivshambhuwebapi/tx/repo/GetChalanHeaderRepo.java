...
public interface GetChalanHeaderRepo extends JpaRepository<GetChalanHeader, Integer> {
	@Query(value = " SELECT " + "        t_chalan_header.*, " + "        m_vehicle.veh_no,"
			...
			+ "        AND t_chalan_header.ex_int1=1  AND t_chalan_header.status!=2 ORDER BY t_chalan_header.status,t_chalan_header.chalan_date DESC  ", nativeQuery = true)
	...
	@Query(value = " SELECT  t_chalan_header.*, m_vehicle.veh_no,m_vehicle.vehicle_name,m_plant.plant_name,"
			...
			+ " AND t_chalan_header.status!=2 ORDER BY t_chalan_header.chalan_date DESC  ", nativeQuery = true)
