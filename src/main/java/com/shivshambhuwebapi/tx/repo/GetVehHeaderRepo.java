...
public interface GetVehHeaderRepo extends JpaRepository<GetVehHeader, Integer> {
	...
	@Query(value = "SELECT h.*,v.vehicle_name,v.veh_comp_name  FROM t_issue_veh_header h,m_vehicle v WHERE h.del_status=1 AND h.veh_id=v.vehicle_id ORDER BY h.mat_veh_header_id DESC", nativeQuery = true)
	List<GetVehHeader> getMatIssueHeadeList();

	@Query(value = "SELECT h.*,v.vehicle_name,v.veh_comp_name  FROM t_issue_veh_header h,m_vehicle v WHERE h.del_status=1 AND h.veh_id=v.vehicle_id AND h.mat_veh_header_id=:matVehHeaderId ORDER BY h.mat_veh_header_id DESC", nativeQuery = true)
	...
	@Query(value = "SELECT h.mat_veh_header_id,h.veh_no,h.veh_id,h.date,h.user_id,SUM(h.veh_total) AS veh_total,SUM(h.veh_qty_total)"
			...
			+ "h.del_status=1 AND h.veh_id=v.vehicle_id AND h.date BETWEEN :fromDate AND :toDate  GROUP BY h.veh_id ORDER BY h.mat_veh_header_id DESC", nativeQuery = true)
