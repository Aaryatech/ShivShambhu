...
public interface GetPoklenReadingRepo extends JpaRepository<GetPoklenReading, Integer> {
	...
	/*
	 * @Query(value =
	 * "SELECT p.*,v.veh_no,v.vehicle_name FROM t_poklen_reading p , m_vehicle v\n"
	 * +
	 * "	WHERE p.del_status=1 AND p.poklen_id=v.vehicle_id AND p.poklen_id=:poklenId "
	 * , nativeQuery = true) List<GetPoklenReading>
	 * getPoklenReadingListByPoklenId(@Param("poklenId") int poklenId);
	 */

	@Query(value = "SELECT p.*,v.veh_no,v.vehicle_name FROM t_poklen_reading p , m_vehicle v	WHERE p.del_status=1 AND p.poklen_id=v.vehicle_id AND p.poklen_id=40 AND p.start_date BETWEEN :fromDate AND :toDate ", nativeQuery = true)
