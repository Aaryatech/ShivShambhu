...
public interface DashPlantRepo extends JpaRepository<DashPlant, Integer> {
	...
	@Query(value = " SELECT coalesce ((\n"
			...
			+ "coalesce (( SELECT SUM(ph.pending_amt) FROM t_pay_recovery_header as ph  WHERE ph.credit_date2 BETWEEN  :fromDate AND :toDate AND ph.del_status = 1 ),'0') AS payment_rec_outstanding_pending", nativeQuery = true)
