...
public interface GetOrderDetailRepo extends JpaRepository<GetOrderDetail, Integer> {
	...
	@Query(value = "SELECT t_order_detail.*, t_cust_po_detail.po_rate,t_cust_po_detail.po_qty,t_cust_po_detail.po_consume_qty,"
			...
			+ "t_cust_po_header.po_no,t_cust_po_header.po_date FROM t_order_header,t_order_detail,t_cust_po_header,"
			+ "t_cust_po_detail,m_item WHERE t_order_detail.item_id=m_item.item_id AND t_order_header.order_id=t_order_detail.order_id "
			+ "AND t_order_detail.po_detail_id=t_cust_po_detail.po_detail_id AND t_cust_po_header.po_id=t_cust_po_detail.po_id and"
			+ " t_order_header.po_id=t_cust_po_header.po_id AND t_order_header.order_id=:orderHeaderId \n"
			+ " ", nativeQuery = true)
