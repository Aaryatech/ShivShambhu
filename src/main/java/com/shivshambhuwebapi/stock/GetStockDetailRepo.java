...
public interface GetStockDetailRepo extends JpaRepository<GetStockDetail, Integer> {
	...
	@Query(value = "SELECT d.stock_det_id,d.stock_id,d.item_id,d.op_qty,i.item_name,i.item_code,u.uom_name,\n"
			...
			+ "d.closing_qty,d.del_status,d.user_id,d.detail_date,d.ex_int1,d.ex_int2,d.ex_float1,\n"
	...
	/*
	 * @Query(value =
	 * "SELECT d.*,i.item_name,i.item_code,u.uom_name  FROM t_stock_det d,m_item_fg i,m_uom u ,t_stock_head h WHERE d.stock_id=h.stock_id AND d.item_id=i.item_id AND u.uom_id=i.uom_id AND h.plant_id=:plantId AND h.status=0"
	 * , nativeQuery = true) List<GetStockDetail>
	 * getStockDetailByPlantAndBetDate(@Param("plantId") int plantId,
	 * 
