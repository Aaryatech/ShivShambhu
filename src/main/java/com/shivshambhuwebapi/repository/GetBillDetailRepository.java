...
public interface GetBillDetailRepository extends JpaRepository<GetBillDetail, Integer>{
	...
	/*@Query(value="select d.*,i.item_name,u.uom_name,t_bill_header.company_id,t_bill_header.ex_var1 as ref_no,c.comp_name,"
			...
			+ "t_bill_header.order_id=o.order_id",nativeQuery=true)
	...
	@Query(value="select d.*,i.item_name,u.uom_name,t_bill_header.company_id,t_bill_header.ex_var1 as ref_no,c.comp_name,\n" + 
			...
			"t_bill_header.company_id=c.company_id and t_bill_header.ex_int1=p.plant_id",nativeQuery=true)
	List<GetBillDetail> getBillDetailsById(@Param("billHeadId")int billHeadId);
