...
public interface GetEnqReportRepo extends JpaRepository<GetEnqReport, Integer> {
	...
	@Query(value = "SELECT h.* ,c.cust_name ,c.cust_mob_no,p.plant_name  FROM t_enq_header h,t_quot_header q,m_customer c ,m_plant p "
			+ "WHERE p.plant_id=h.plant_id AND h.ex_int1=1 AND q.status=0 AND c.cust_id=h.cust_id AND enq_status=0 AND h.enq_date "
			+ "BETWEEN :fromDate AND :toDate AND h.plant_id IN(:plantIdList) ", nativeQuery = true)
