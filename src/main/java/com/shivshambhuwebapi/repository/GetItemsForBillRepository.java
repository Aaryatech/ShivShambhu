...
public interface GetItemsForBillRepository extends JpaRepository<GetItemsForBill, Integer> {
	...
	@Query(value = "select d.chalan_detail_id,	h.chalan_id,	h.chalan_no,\n" + 
			...
			"				and h.chalan_id IN (:chalanId) and d.status IN(0,1) and d.del_status=1 and od.po_detail_id=t_cust_po_detail.po_detail_id group by  d.chalan_detail_id ", nativeQuery = true)
