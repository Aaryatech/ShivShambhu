...
//			+ " BETWEEN :fromDate AND :toDate AND h.company_id IN(:companyIdList) AND h.ex_int1 IN(:plantIdList)", nativeQuery = true)
	...
	@Query(value = "SELECT  h.*,c.cust_name,c.cust_mob_no,l.plant_name,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h  ,m_customer c,"
			+ "m_plant l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1 AND  c.cust_id=h.cust_id AND l.plant_id=h.ex_int1 AND h.bill_date"
...
//			+ " BETWEEN :fromDate AND :toDate AND h.company_id IN(:companyIdList)", nativeQuery = true)
	...
	@Query(value = "SELECT h.*,c.cust_name,c.cust_mob_no ,l.plant_name, sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt,sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h,m_customer c,m_plant l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1  AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1 AND h.bill_date BETWEEN :fromDate AND :toDate GROUP BY h.bill_head_id", nativeQuery = true)
	// SELECT h.*,c.cust_name,c.cust_mob_no,p.proj_name,l.plant_name FROM
	// t_bill_header h,m_project p ,m_customer c,"
	// + "m_plant l WHERE h.del_status=1 AND p.proj_id=h.proj_id AND
	// c.cust_id=h.cust_id AND l.plant_id=h.ex_int1 AND h.bill_date "
...
//			+ ":fromDate AND :toDate GROUP BY h.cust_id\n", nativeQuery = true)
	...
	@Query(value = "SELECT  h.bill_head_id,h.bill_no,h.bill_date,h.cust_id,h.proj_id,h.cost_segment,h.payment_term_id,h.company_id,"
			...
			+ "as total_amt,h.acc_id,h.order_id,h.del_status,h.ex_int1,h.ex_int2,h.ex_int3,h.ex_var1,h.ex_var2,h.ex_var3,h.ex_float1,"
			+ "h.ex_float2,c.cust_name,c.cust_mob_no,l.plant_name,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h,m_customer c,m_plant "
			+ "l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1  AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1 AND h.bill_date BETWEEN "
			+ ":fromDate AND :toDate GROUP BY h.cust_id\n", nativeQuery = true)
...
//			+ "BETWEEN :fromDate AND :toDate AND h.cust_id IN(:custIdList) GROUP BY h.cust_id", nativeQuery = true)
	...
	@Query(value = "SELECT  h.bill_head_id,h.bill_no,h.bill_date,h.cust_id,h.proj_id,h.cost_segment,h.payment_term_id,h.company_id,"
			...
			+ " total_amt,h.acc_id,h.order_id,h.del_status,h.ex_int1,h.ex_int2,h.ex_int3,h.ex_var1,h.ex_var2,h.ex_var3,h.ex_float1,"
			+ "h.ex_float2,c.cust_name,c.cust_mob_no,p.proj_name,l.plant_name  ,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h,m_customer c,m_plant "
...
//			+ "AND h.bill_date BETWEEN :fromDate AND :toDate AND h.cust_id IN(:custIdList) AND h.ex_int1=:plantId GROUP BY h.cust_id", nativeQuery = true)
	...
	@Query(value = "SELECT  h.bill_head_id,h.bill_no,h.bill_date,h.cust_id,h.proj_id,h.cost_segment,h.payment_term_id,"
			...
			+ "h.ex_var3,h.ex_float1,h.ex_float2,c.cust_name,c.cust_mob_no, l.plant_name  ,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h "
			+ " ,m_customer c,m_plant l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1   AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1 "
			+ "AND h.bill_date BETWEEN :fromDate AND :toDate AND h.cust_id IN(:custIdList) AND h.ex_int1=:plantId GROUP BY h.cust_id", nativeQuery = true)
...
//			+ ",m_plant l WHERE h.del_status=1 AND p.proj_id=h.proj_id AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1  AND h.cust_id=:custId", nativeQuery = true)
	...
	@Query(value = "SELECT  h.*,c.cust_name,c.cust_mob_no, l.plant_name ,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h,m_customer c"
			+ ",m_plant l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1 AND  c.cust_id=h.cust_id AND l.plant_id=h.ex_int1  AND h.cust_id=:custId GROUP BY h.bill_head_id", nativeQuery = true)
...
//			+ ",m_plant l WHERE h.del_status=1 AND p.proj_id=h.proj_id AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1  AND h.cust_id=:custId AND h.bill_date BETWEEN :fromDate AND :toDate", nativeQuery = true)
	...
	@Query(value = "SELECT  h.*,c.cust_name,c.cust_mob_no, l.plant_name  ,sum(d.sgst_amt) as sgst_amt,sum(d.cgst_amt) as cgst_amt, sum(d.igst_amt) as igst_amt FROM t_bill_detail d, t_bill_header h ,m_customer c"
			+ ",m_plant l WHERE h.bill_head_id=d.bill_head_id AND h.del_status=1   AND c.cust_id=h.cust_id AND l.plant_id=h.ex_int1  AND h.cust_id=:custId AND h.bill_date BETWEEN :fromDate AND :toDate GROUP BY h.bill_head_id", nativeQuery = true)
